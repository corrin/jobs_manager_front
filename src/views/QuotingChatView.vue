          ...
          <div class="space-y-4">
            ...
            >
              ...
              >
                ...
                <div
                  v-if="message.senderId !== currentUserId && message.metadata?.mode"
                  class="mt-2"
                >
                  <span
...
const handleFileUpload = async (event: Event) => {
  ...
  if (!jobContext.value?.jobId) {
    toast.error('No job context', {
      description: 'Cannot upload files without a job context',
    })
    return
  }
  ...
  try {
    ...
    const response = await jobService.uploadJobFiles(jobContext.value.jobId, fileArray)

  ...
  <Dialog :open="isOpen" @update:open="handleOpenChange">
    <DialogContent class="max-w-7xl h-[90vh] overflow-y-auto">
      ...
      <div class="space-y-6">
            ...
            <div class="mb-4 bg-gray-50 p-3 rounded-lg">
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span
                    :class="getHoursVariancePercentage(job) > 0 ? 'text-red-600' : 'text-green-600'"
                  >
                    {{ getHoursVariancePercentage(job) > 0 ? 'Over' : 'Under' }}
                    {{ Math.abs(getHoursVariancePercentage(job)) }}%
                  </span>
                </div>
            ...
            <div class="grid grid-cols-2 gap-2 mb-4">
              ...
              <div class="bg-emerald-50 p-3 rounded text-center">
                ...
                <p class="text-sm font-bold text-emerald-900">
                  ${{ formatCurrency(getJobProfit(job)) }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div
          class="fixed bottom-4 left-4 bg-white border border-gray-300 rounded-lg shadow-lg p-4 max-w-xs z-50"
        >
...
import { Target, Activity, DollarSign, TrendingUp, Users, Search, FileX } from 'lucide-vue-next'
...
const loadJobs = async () => {
  ...
  try {
    ...
    const allJobs = [...(response.active_jobs || [])]
    ...
    jobs.value = allJobs.map((job) => {
      ...
      return {
        ...
        total_estimated_hours:
          metricsForJob.total_estimated_hours ||
          job.total_estimated_hours ||
          job.estimated_hours ||
          0,
        total_actual_hours:
          metricsForJob.total_actual_hours || job.total_actual_hours || job.actual_hours || 0,
        total_revenue:
          metricsForJob.total_revenue || job.total_revenue || job.revenue || job.total_value || 0,

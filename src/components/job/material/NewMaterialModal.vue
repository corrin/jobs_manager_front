...
const chargePercentage = ref<number>(props.companyDefaults?.materials_markup !== undefined ? props.companyDefaults.materials_markup * 100 : 0);
...
const resetForm = () => {
  ...
  chargePercentage.value = props.companyDefaults?.materials_markup !== undefined
...
watch(() => props.companyDefaults, (newDefaults) => {
    ...
    if (chargePercentage.value === 0 || (newDefaults?.materials_markup !== undefined)) {
        chargePercentage.value = newDefaults?.materials_markup !== undefined
                               ? newDefaults.materials_markup * 100
                               : 0;
    }
}, { deep: true });
...
<template>
  <Dialog :open="internalOpen" @update:open="internalOpen = $event">
    <DialogContent class="sm:max-w-[580px]">
      ...
      <div v-if="validationErrors.general" class="bg-red-50 border border-red-200 rounded-md p-3 mb-4">
      ...
      <div class="grid gap-6 py-4">
        ...
        <div class="space-y-4 py-3">
            <h3 class="text-md font-semibold text-gray-700 dark:text-gray-300 border-b pb-2">Cost to You</h3>
            <div class="grid grid-cols-2 gap-4 items-end">
                ...
                <div class="grid gap-1 text-right">
                    ...
                    <p class="text-xl font-semibold h-10 flex items-center justify-end">{{ formatCurrency(totalCost) }}</p>
                </div>
            </div>
        </div>

        <div class="space-y-4 py-3">
            <h3 class="text-md font-semibold text-gray-700 dark:text-gray-300 border-b pb-2">What You'll Charge</h3>
            <div class="grid grid-cols-2 gap-4 items-end">
                <div class="grid gap-2">
                 ...
                 <div class="grid gap-1 text-right">
                    ...
                    <p class="text-xl font-semibold h-10 flex items-center justify-end">{{ formatCurrency(totalChargeAmount) }}</p>
                </div>
            </div>
             <p v-if="chargePercentage !== null && chargePercentage !== undefined" class="text-xs text-muted-foreground text-center">

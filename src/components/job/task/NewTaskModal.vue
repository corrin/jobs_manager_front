  ...
  <Dialog :open="internalOpen" @update:open="internalOpen = $event">
    <DialogContent class="sm:max-w-[525px]">
      ...
      <div class="grid gap-6 py-4">
        ...
        <div v-if="effectiveHourlyRate > 0" class="text-sm text-muted-foreground px-1 py-2 border-t border-b">
            Effective Hourly Rate: {{ formatCurrency(effectiveHourlyRate) }}
            (Base Rate: {{ formatCurrency(props.jobChargeOutRate || props.companyDefaults?.charge_out_rate || 0) }} x {{ chargeMultiplier }})
        </div>

        <div class="p-4 my-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg text-center shadow">
          <p class="text-xs text-muted-foreground uppercase tracking-wider">Estimated Total Value</p>
          <p class="text-3xl font-semibold text-slate-900 dark:text-slate-50">
            {{ formatCurrency(totalEstimatedAmount) }}
          </p>
          <p v-if="(props.companyDefaults?.time_markup ?? 0) > 0" class="text-xs text-muted-foreground">
            (Includes {{ (props.companyDefaults?.time_markup ?? 0) * 100 }}% time markup)
          </p>
        </div>

        <div v-if="validationErrors.general" class="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-md">

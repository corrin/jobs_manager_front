<template>
  <div class="quote-import-dialog">
            ...
            >
              <div class="flex items-start">
                <svg
                  ...
                  viewBox="0 0 20 20"
                >
                  <path v-if="issue.severity === 'error'" fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
          ...
          <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div class="text-center">
                <div class="text-2xl font-bold text-blue-600">{{ previewData.preview.diff_preview.next_revision }}</div>
                <div class="text-sm text-gray-600">Next Revision</div>
              </div>
              <div class="text-center">
                <div class="text-2xl font-bold text-gray-900">{{ previewData.preview.diff_preview.total_changes }}</div>
                <div class="text-sm text-gray-600">Total Changes</div>
              </div>
              <div class="text-center">
                <div class="text-2xl font-bold text-green-600">+{{ previewData.preview.diff_preview.additions_count }}</div>
                <div class="text-sm text-gray-600">Additions</div>
              </div>
              <div class="text-center">
                <div class="text-2xl font-bold text-yellow-600">~{{ previewData.preview.diff_preview.updates_count }}</div>
                <div class="text-sm text-gray-600">Updates</div>
              </div>
              <div class="text-center">
                <div class="text-2xl font-bold text-red-600">-{{ previewData.preview.diff_preview.deletions_count }}</div>
    ...
    <div v-if="importResult" class="result-section">
      <div v-if="importResult.success" class="success bg-green-50 border border-green-200 rounded-lg p-6">
        <div class="flex items-start">
          <div class="flex-shrink-0">
          ...
          <div class="ml-3">
            ...
            <div class="mt-2 text-sm text-green-700">
              ...
              <div v-if="importResult.cost_set" class="mt-3 space-y-1">
                ...
                <p><strong>Total Cost:</strong> ${{ importResult.cost_set.summary.cost.toLocaleString() }}</p>
                <p v-if="importResult.changes">
                  <strong>Changes:</strong>
                  +{{ importResult.changes.additions }} 
                  ~{{ importResult.changes.updates }} 
      ...
      <div v-else class="error bg-red-50 border border-red-200 rounded-lg p-6">
        <div class="flex items-start">
          ...
          <div class="ml-3">
            ...
            <p class="mt-1 text-sm text-red-700">{{ importResult.error || 'Unknown error occurred' }}</p>
          </div>
        </div>
      </div>
    </div>
